...
import {
  ...
  mergeConcurrently
} from '../../src/combinator/mergeConcurrently'
...
describe('mergeConcurrently', () => {
  it('should be identity for 1 stream', () => {
    ...
    return collectEventsFor(n, take(n, s)).then(eq(expected))
  })

  it('should merge all when number of streams <= concurrency', () => {
    const streams = [
      periodicConstant(1, 1),
      periodicConstant(1, 2),
      periodicConstant(1, 3)
    ]
    ...
    return collectEventsFor(n, take(n * streams.length, s)).then(eq(expected))
  })
  ...
  it('should merge up to concurrency', () => {
    ...
    const streams = [
      take(n, periodicConstant(1, 1)),
      take(n, periodicConstant(1, 2)),
      take(n, periodicConstant(1, 3))
    ]
    ...
    return collectEventsFor(m * n, take(n * streams.length, s)).then(
      eq(expected)
    )
  })
})
