...
describe('zipArray', function () {
  ...
  it('given array containing canonical empty, should return canonical empty', () => {
    const s = zipArray(Array, [now(0), empty(), now(1)])
    assert(isCanonicalEmpty(s))
  })

  it('should invoke f for each tuple', function () {
    const n = 3
    const s = zipArray(Array, [makeEvents(1, n), makeEvents(2, n), makeEvents(3, n)])
  ...
  it('should end when shortest stream ends', function () {
    ...
    return collectEventsFor(n, zipArray(Array, [a, b, c]))
      .then(eq([
        { time: 0, value: [0, 0, 0] },
        { time: 1, value: [1, 1, 1] },
        { time: 2, value: [2, 2, 2] }
