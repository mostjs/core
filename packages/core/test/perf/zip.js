...
suite
  .add(
    ...
    function(deferred) {
      runners.runMost(
        deferred,
        reduce(add, 0, zip(add, fromArray(b), fromArray(a)))
      )
    },
    options
  )
  .add(
    ...
    function(deferred) {
      runners.runRx(
        ...
        rx.Observable
          .fromArray(a)
          .zip(rx.Observable.fromArray(b), add)
          .reduce(add, 0)
      )
    },
    options
  ...
  .add(
    ...
    function(deferred) {
      runners.runRx5(
        ...
        rxjs.Observable
          .from(a)
          .zip(rxjs.Observable.from(b), add)
          .reduce(add, 0)
      )
    },
    options
  ...
  .add(
    ...
    function(deferred) {
      runners.runBacon(
        deferred,
        bacon
          .zipWith(add, bacon.fromArray(a), bacon.fromArray(b))
          .reduce(0, add)
      )
    },
    options
