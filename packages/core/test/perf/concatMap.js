...
const xstreamFlattenSequentially = require('xstream/extra/flattenSequentially')
  .default
...
suite
  .add(
    ...
    function(deferred) {
      runners.runMost(
        deferred,
        reduce(sum, 0, concatMap(fromArray, fromArray(a)))
      )
    },
    options
  )
  .add(
    ...
    function(deferred) {
      runners.runRx(
        ...
        rx.Observable
          .fromArray(a)
          .concatMap(rx.Observable.fromArray)
          .reduce(sum, 0)
      )
    },
    options
    ...
    function(deferred) {
      runners.runXstream(
        ...
        xs
          .fromArray(a)
          .map(xs.fromArray)
          .compose(xstreamFlattenSequentially)
          .fold(sum, 0)
          .last()
      )
    },
