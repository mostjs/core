...
export const withItems = (array, stream) =>
  zipItems(keepLeft, array, stream)

export const zipItems = (f, array, stream) =>
  array.length === 0 || stream === empty()
    ? empty()
    : new ZipItems(f, array, stream)
...
class ZipItems {
  ...
  run (sink, scheduler) {
    return this.source.run(new ZipItemsSink(this.f, this.values, sink), scheduler)
  }
}
